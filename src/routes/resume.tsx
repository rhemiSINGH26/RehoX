...
export function ResumePage() {
  ...
  const highConfidenceCount = resume?.skills.filter((s) => s.confidence === "high").length || 0;
  const categoriesPresent = Array.from(new Set(resume?.skills.map((s) => s.category_code) || []));
  ...
  if (resume) {
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-8 py-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
        ...
        {/* Candidate Profile Hero Banner Card */}
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2.5 pt-1">
                ...
                {resume.email && (
                  <span className="inline-flex items-center rounded-lg bg-panel border border-line/60 px-3.5 py-1 text-xs font-medium text-ink-text">
                    ✉️ {resume.email}
                  </span>
                )}
                {resume.cgpa && (
        ...
        {/* Section 1: Candidate Education, Projects & Experience Summary */}
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-line/60 bg-panel/50 p-5 space-y-3">
            <div className="flex items-center gap-2 border-b border-line/40 pb-2">
              ...
              <h3 className="font-display text-base font-bold text-ink-text">Education & Degree</h3>
            </div>
          ...
          <div className="rounded-2xl border border-line/60 bg-panel/50 p-5 space-y-3">
            ...
            {resume.experience && resume.experience.length > 0 ? (
              ...
              <p className="text-xs text-muted-text">No formal work experience listed.</p>
            )}
          </div>
          ...
          <div className="rounded-2xl border border-line/60 bg-panel/50 p-5 space-y-3 md:col-span-2">
            ...
            {resume.projects && resume.projects.length > 0 ? (
              ...
              <p className="text-xs text-muted-text">No technical projects extracted.</p>
            )}
          </div>
        </div>

        {/* Section 2: Skill Signal Breakdown */}
        <div className="space-y-4">
          ...
          {resume.skills.length === 0 ? (
            <div className="rounded-2xl border border-line bg-panel/30 p-12 text-center text-muted-text">
              No skill signals detected. Try uploading a more comprehensive resume.
            </div>
          ) : (
  ...
  // VIEW 3: INITIAL UPLOAD VIEW (When no resume uploaded yet)
  return (
