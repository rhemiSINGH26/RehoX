...
export const Route = createFileRoute("/resume")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Upload a resume to see the skills it shows — with evidence and confidence.",
      },
      { property: "og:title", content: "Resume Parsing · RehoX" },
      {
        property: "og:description",
        content: "Turn a resume into structured skills, projects, and experience.",
      },
    ],
  }),
      ...
      <div className="space-y-4">
        <header>
          <div className="mono text-xs uppercase tracking-widest text-brass">
            Step 02 · Resume Parsing
          </div>
          <h1 className="mt-2 font-display text-3xl font-bold">
            Upload a resume to see what it shows.
          </h1>
        ...
        <div className="rounded-xl border border-line bg-panel/40 p-4">
          ...
          >
            ...
            {SAMPLE_RESUMES.map((r) => (
              <option key={r.source_file} value={r.source_file}>
                {r.displayName}
              </option>
            ))}
          </select>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            <PaperCard title={`Resume · ${resume.source_file}`} subtitle={resume.role}>
              <div className="mb-3 mono text-[10px] uppercase tracking-widest text-black/50">
                Skills
              </div>
              {resume.skills.map((s, i) => (
                <PaperSkillRow key={i} skill={s} />
              ))}
