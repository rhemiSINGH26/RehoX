...
export const Route = createFileRoute("/resume-builder")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Build, score, and optimize your resume for ATS systems with live AI feedback." },
    ],
  }),
...
export function ResumeBuilderPage() {
  ...
  const [resume, setResume] = useState<ATSResume>(() => ({
    ...
    summary:
      ...
      `Results-driven Software Engineer specializing in ${profile?.skills.slice(0, 3).map((s) => s.skill_name).join(", ") || "full-stack development and distributed systems"}. Proven track record of architecting scalable applications.`,
    skills: profile?.skills.map((s) => s.skill_name) || [
      "TypeScript", "React", "Node.js", "Python", "System Design", "AWS", "Docker", "PostgreSQL"
    ],
    experience: [
      {
        company: jd?.company && jd.company !== "Unknown" ? `${jd.company} Target Labs` : "Tech Innovations Inc.",
    ...
    education: [
      {
        institution: profile?.education || resumeSource?.education || "Bachelor of Science in Computer Science",
  ...
  const atsResult = useMemo(() => evaluateAtsResume(resume, jd), [resume, jd]);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-line pb-5">
        <div>
          <div className="mono text-xs uppercase tracking-widest text-brass font-semibold">Step 07 · ATS Resume Engine</div>
          <h1 className="mt-1 font-display text-3xl font-bold text-ink-text">ATS Resume Builder & Feedback</h1>
        ...
        <div className="space-y-6 sticky top-20">
          ...
          <div className="rounded-2xl border border-line/60 bg-panel/50 p-5 space-y-3 shadow-sm">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between items-center">
                ...
                <span className="mono font-bold text-brass">{atsResult.keyword_match_score}%</span>
              </div>
              <div className="h-2 w-full rounded-full bg-ink overflow-hidden border border-line/40">
                <div className="h-2 rounded-full bg-brass" style={{ width: `${atsResult.keyword_match_score}%` }} />
              </div>

              <div className="flex justify-between items-center pt-2">
                <span className="text-muted-text font-medium">Formatting & Readability:</span>
                <span className="mono font-bold text-emerald-400">{atsResult.formatting_score}%</span>
              </div>
              <div className="h-2 w-full rounded-full bg-ink overflow-hidden border border-line/40">
                <div className="h-2 rounded-full bg-emerald-500" style={{ width: `${atsResult.formatting_score}%` }} />
              </div>

              <div className="flex justify-between items-center pt-2">
                <span className="text-muted-text font-medium">Action Verbs & Impact:</span>
                <span className="mono font-bold text-amber-400">{atsResult.impact_action_score}%</span>
              </div>
              <div className="h-2 w-full rounded-full bg-ink overflow-hidden border border-line/40">
                <div className="h-2 rounded-full bg-amber-500" style={{ width: `${atsResult.impact_action_score}%` }} />
              </div>
            </div>
          </div>
          ...
          {atsResult.missing_keywords.length > 0 && (
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-5 space-y-3 shadow-sm">
              ...
              <p className="text-[11px] text-muted-text">Include these key terms to boost ATS match:</p>
              <div className="flex flex-wrap gap-1.5">
                {atsResult.missing_keywords.map((kw, i) => (
                  <span key={i} className="mono text-[10px] rounded-lg bg-ink px-2 py-0.5 text-amber-400 border border-amber-500/20">
                    + {kw}
                  </span>
                ))}
              </div>
        ...
        <div className="rounded-3xl border border-line/60 bg-panel/60 p-8 space-y-6 shadow-xl backdrop-blur-md">
          ...
          <div className="border-b border-line/40 pb-6 space-y-4">
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                <label className="block text-[11px] mono uppercase text-muted-text font-bold mb-1">Full Name</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[11px] mono uppercase text-muted-text font-bold mb-1">Email Address</label>
                <input
                  type="email"
                  value={resume.email}
            ...
            <div>
              <label className="block text-[11px] mono uppercase text-muted-text font-bold mb-1">Professional Summary</label>
              <textarea
                rows={3}
          ...
          <div className="border-b border-line/40 pb-6 space-y-3">
            <label className="block text-[11px] mono uppercase text-brass font-bold">Technical Skills & Competencies</label>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between border-b border-line/40 pb-2">
              <label className="block text-[11px] mono uppercase text-brass font-bold">Work Experience & Impact</label>
              <span className="mono text-[10px] text-muted-text">⚡ AI Bullet Enhancer Enabled</span>
            </div>

            {resume.experience.map((exp, expIdx) => (
              <div key={expIdx} className="rounded-2xl border border-line/60 bg-ink/40 p-5 space-y-4 shadow-sm">
                <div className="grid gap-3 md:grid-cols-2">
                  <div>
                  ...
                  <div>
                    <label className="block text-[10px] mono text-muted-text mb-1">Role Title</label>
                    <input
                      type="text"
                      value={exp.role}
                      onChange={(e) => {
                ...
                <div className="space-y-3">
                  <label className="block text-[10px] mono text-muted-text">Achievement Bullet Points</label>
