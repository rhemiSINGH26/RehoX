...
import { COMPANIES, resolveCompanyName } from "@/lib/rehox/company-expectations";
import { readinessLabel } from "@/lib/rehox/compute";
import { rehoxStore, useRehox } from "@/lib/rehox/store";
import { checkTalent } from "@/lib/rehox/talent-check";
import { CATEGORY_LABEL, type TalentCheckResult } from "@/lib/rehox/types";
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Talent Check · RehoX" },
      {
        ...
        content:
          "The dial shows required level as the ring and your level as the fill. Gaps are visible as empty space.",
      },
    ],
  }),
...
function TalentCheckPage() {
  ...
  const defaultCompany = resolveCompanyName(jd?.company ?? "") ?? COMPANIES[0] ?? "";
  ...
  useEffect(() => {
    const resolvedJdCompany = resolveCompanyName(jd?.company ?? "");
    if (resolvedJdCompany) setCompany(resolvedJdCompany);
  }, [jd?.company]);
  ...
  useEffect(() => {
    ...
    if (!company) {
      setResult(null);
      setError("No companies are available in the local expectations snapshot.");
      setLoading(false);
      return;
    }

    if (!profile.competency_levels) {
      setResult(null);
      setError("Candidate profile is missing competency_levels for Talent Check.");
      setLoading(false);
      return;
    }
    ...
    checkTalent({
      data: {
        company,
        candidate_profile: { competency_levels: profile.competency_levels },
      },
    })
      .then((talentCheck) => {
        if (cancelled) return;
        setResult(talentCheck);
        rehoxStore.set({ talentCheck });
      })
      .catch((err) => {
        if (cancelled) return;
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="mono text-xs uppercase tracking-widest text-brass">
            Step 04 · Talent Check
          </div>
          <h1 className="mt-2 font-display text-3xl font-bold">Are you at the company's bar?</h1>
        </div>
        <label className="text-sm">
          <span className="mono block text-[10px] uppercase tracking-widest text-muted-text">
            Company
          </span>
          <select
            value={company}
          ...
          >
            {COMPANIES.map((candidateCompany) => (
              <option key={candidateCompany} value={candidateCompany}>
                {candidateCompany}
              </option>
            ))}
          </select>
      ...
      {error && (
        <div className="rounded-xl border border-alert-coral/40 bg-alert-coral/5 px-4 py-3 text-sm text-alert-coral">
          <span className="mono font-semibold">Talent Check unavailable · </span>
          {error}
        </div>
      )}

      {loading && (
        <div className="rounded-xl border border-line bg-panel/30 px-4 py-3 text-sm text-muted-text">
          Running deterministic Talent Check...
        </div>
      )}

      {!result ? (
        <div className="rounded-xl border border-dashed border-line bg-panel/20 p-10 text-center text-sm text-muted-text">
          Talent Check needs a selected company and candidate competency levels from Profile
          Builder.
        </div>
      ) : (
        <>
          <div className="grid gap-8 md:grid-cols-[minmax(0,1fr),320px]">
            ...
            <div className="rounded-xl border border-line bg-panel/30 p-6">
              ...
              <div className="mt-1 font-display text-6xl font-bold">{result.readiness_score}</div>
              ...
              <div className="mt-6 border-t border-line pt-4 text-sm text-muted-text">
                {gapCount === 0
                  ? "No gaps against this company's bar."
          ...
          {result.explanation && (
            <div className="rounded-xl border border-line bg-panel/30 p-4">
              ...
              <p className="mt-2 text-sm text-muted-text">{result.explanation}</p>
            </div>
          )}
          ...
          {result.top_priorities && result.top_priorities.length > 0 && (
            <div className="rounded-xl border border-line bg-panel/30 p-4">
              ...
              <div className="mt-3 space-y-2 text-sm text-muted-text">
                {result.top_priorities.map((priority) => (
                  <div
                    key={priority.category_code}
                    className="flex items-center justify-between rounded-md border border-line/70 px-3 py-2"
                  >
                    <span>{CATEGORY_LABEL[priority.category_code]}</span>
                    <span className="mono text-brass">
                      {priority.severity} · priority {priority.priority}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div className="rounded-xl border border-line bg-panel/30 overflow-hidden">
            ...
            {sortedGap.map((row) => (
              <div
                ...
                className="grid grid-cols-[80px,1fr,80px,80px,80px] items-center gap-3 border-t border-line px-4 py-2.5 text-sm"
              >
                ...
                {row.gap ? (
                  <span className="flex items-center gap-1.5 text-alert-coral">
                    <span className="h-1.5 w-1.5 rounded-full bg-alert-coral" />
                    gap
                  </span>
                ) : (
...
export function EmptyState({
  ...
  return (
    <div className="mx-auto max-w-lg rounded-xl border border-dashed border-line bg-panel/30 p-10 text-center">
      ...
      <Link
        to={ctaTo}
        className="mt-6 inline-block rounded-md bg-brass px-4 py-2 text-sm font-medium text-primary-foreground hover:brightness-110"
      >
        {ctaLabel}
      </Link>
    </div>
