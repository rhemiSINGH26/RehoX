...
function normalizeApiResponse(
  ...
  return {
    ...
    category_scores: Array.isArray(payload.category_scores)
      ? payload.category_scores.filter((entry): entry is { category: string; score: number } =>
          Boolean(
            entry &&
            typeof entry === "object" &&
            "category" in entry &&
...
export const Route = createFileRoute("/skill-match")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Compare your profile to a specific JD — with a match score and a gap list.",
      },
      { property: "og:title", content: "Skill Match · RehoX" },
      {
        property: "og:description",
        content: "See exactly which JD skills you match and which you're missing.",
      },
    ],
  }),
...
function SkillMatchPage() {
  ...
  // Auto-infer profile from resume if storedProfile is not set yet
  ...
  const fallbackResult = useMemo(
    () => (profile && jd ? runSkillMatch(profile, jd) : null),
    [profile, jd],
  );
  ...
  useEffect(() => {
    ...
    async function loadMatch() {
      if (!profile || !jd || jd.skills.length === 0) {
        setApiResult(null);
        return;
      }
      ...
      try {
        const payload = {
          candidate_profile: {
            ...
            skills: profile.skills.map((skill) => ({
              ...
              evidence: skill.evidence || "Skill signal",
              confidence: skill.confidence || "high",
            })),
          },
          jd: {
            company: jd.company || "Target",
            role: jd.role || "Role",
            skills: jd.skills.map((skill) => ({
              skill_name: skill.skill_name,
              category_code: skill.category_code,
              evidence: skill.evidence || "Requirement",
              confidence: skill.confidence || "high",
            })),
          },
        };
        ...
        const targetEndpoint =
          typeof window !== "undefined" ? "/api/match" : "http://127.0.0.1:8000/api/match";
        ...
        // Fallback to local match calculation
      }
    }
  ...
  if (!profile) {
    return (
      <EmptyState
        title="No candidate profile or resume uploaded."
        ...
        <div className="rounded-3xl border border-line/60 bg-panel/50 p-6 flex flex-col justify-between space-y-4 shadow-sm">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-text leading-relaxed font-medium">
              {r.summary ||
                `Candidate shows a ${r.match_score}% skill alignment against ${jd.role}. Candidate matches ${r.matched_skills.length} out of ${jd.skills.length} required skill signals.`}
            </p>
          </div>
      ...
      {r.category_scores && r.category_scores.length > 0 && (
        <section className="rounded-2xl border border-line/60 bg-panel/50 p-6 space-y-4 shadow-sm">
          <h2 className="font-display text-lg font-bold text-ink-text">
            Category Skill Match Breakdown
          </h2>
          <div className="grid gap-3.5 md:grid-cols-2">
            {r.category_scores.map((entry) => (
              <div
                key={entry.category}
                className="rounded-xl border border-line/60 bg-ink/50 p-4 space-y-2"
              >
                ...
                <div className="h-2.5 w-full rounded-full bg-panel overflow-hidden border border-line/40">
                  <div
                    className="h-2.5 rounded-full bg-brass transition-all duration-500"
      ...
      <section className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-6 space-y-4 shadow-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-lg font-bold text-ink-text">
              Missing Skill Requirements ({r.missing_skills.length})
            </h2>
          </div>
        </div>
        ...
        {r.missing_skills.length === 0 ? (
          <div className="text-xs text-emerald-400 font-semibold">
            All skill requirements met! Candidate covers all required competencies.
          </div>
        ) : (
          <div className="grid gap-3 md:grid-cols-2">
            {r.missing_skills.map((s, i) => (
              <div
                key={i}
                className="rounded-xl border border-line/60 bg-panel/80 p-4 space-y-2 shadow-sm"
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="mono text-[10px] uppercase text-muted-text">
                    {CATEGORY_LABEL[s.category_code]}
                  </span>
                </div>
                {s.evidence && (
      ...
      <section className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-6 space-y-4 shadow-sm">
        ...
        <div className="grid gap-2.5 md:grid-cols-3">
          {r.matched_skills.map((s, i) => (
            <div
              key={i}
              className="flex items-center gap-2.5 rounded-xl border border-line/60 bg-panel/80 px-4 py-3 text-xs shadow-sm"
            >
          ...
          {r.matched_skills.length === 0 && (
            <div className="text-xs text-muted-text">No matches found yet.</div>
          )}
        </div>
      </section>
    </div>
