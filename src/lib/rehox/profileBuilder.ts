...
const SKILL_TAXONOMY: {
  ...
  {
    pattern: /^(python|python3|py|python programming|python\s*\(pandas\))$/i,
    canonical: "Python",
    category: "COD",
  },
  { pattern: /^(c\+\+|cpp|c\+\+11|c\+\+17|c\+\+20)$/i, canonical: "C++", category: "COD" },
  {
    pattern: /^(java|java\s*8|java\s*11|java programming|java basics)$/i,
    canonical: "Java",
    category: "COD",
  },
  ...
  {
    pattern: /^(javascript|typescript|js|ts|node|node\.js|react|vue|angular)$/i,
    canonical: "JavaScript / TypeScript",
    category: "COD",
  },

  {
    pattern: /(data structures|dsa|algorithms|data structures & algorithms|problem solving)/i,
    canonical: "Data Structures & Algorithms",
    category: "DSA",
  },
  {
    pattern: /(object-oriented|object oriented|ood|oops|oop)/i,
    canonical: "Object-Oriented Design",
    category: "OOD",
  },
  {
    pattern:
      /(aptitude|quantitative|statistics|statistical|math|numerical reasoning|excel \/ aptitude)/i,
    canonical: "Quantitative Aptitude & Stats",
    category: "APTI",
  },
  {
    pattern: /(communication|client communication|stakeholder|presentation|technical writing)/i,
    canonical: "Communication",
    category: "COMM",
  },
  {
    pattern:
      /(machine learning|ml|ai|artificial intelligence|deep learning|power bi|pandas|scikit-learn|sklearn|tensorflow|pytorch|recommender)/i,
    canonical: "Machine Learning / AI",
    category: "AI",
  },
  {
    pattern:
      /(cloud|aws|gcp|azure|amazon web services|google cloud|azure data|synapse|cloud platforms)/i,
    canonical: "Cloud Infrastructure",
    category: "CLOUD",
  },
  {
    pattern: /(sql|oracle sql|pl-sql|pl\/sql|postgres|postgresql|mysql|sql server|database)/i,
    canonical: "SQL & Databases",
    category: "SQL",
  },
  {
    pattern: /(software engineering|swe|agile|sdlc|ci\/cd|git|code review|swe practices)/i,
    canonical: "Software Engineering Practices",
    category: "SWE",
  },
  {
    pattern:
      /(system design|distributed systems|microservices|scalable architecture|raft|distributed)/i,
    canonical: "System Design & Distributed Systems",
    category: "SYSD",
  },
  {
    pattern: /(networking|tcp\/ip|http|socket|network topology|computer networks)/i,
    canonical: "Computer Networking",
    category: "NETW",
  },
  {
    pattern:
      /(operating systems|os|linux|unix|linux shell|bash|shell scripting|os internals|syscall tracer)/i,
    canonical: "Operating Systems & Linux",
    category: "OS",
  },
];
...
export function normalizeSkill(input: Skill | string): Skill {
  ...
  if (matchedCategory === "OTHER") {
    ...
    if (lower.includes("sql") || lower.includes("db")) matchedCategory = "SQL";
    ...
    else if (lower.includes("design") || lower.includes("system") || lower.includes("microservice"))
      matchedCategory = "SYSD";
    else if (lower.includes("ml") || lower.includes("ai") || lower.includes("model"))
      matchedCategory = "AI";
    else if (
      lower.includes("test") ||
      lower.includes("ci") ||
      ...
      matchedCategory = "SWE";
    else if (lower.includes("net") || lower.includes("ip") || lower.includes("web"))
      matchedCategory = "NETW";
    else if (
      ...
      matchedCategory = "OS";
    else if (lower.includes("alg") || lower.includes("tree") || lower.includes("graph"))
      matchedCategory = "DSA";
    else if (lower.includes("code") || lower.includes("script") || lower.includes("dev"))
      matchedCategory = "COD";
  }
...
export function mergeDuplicateSkills(skills: Skill[]): Skill[] {
  ...
  for (const sk of skills) {
    ...
    if (!existing) {
      ...
      const bestConf =
        confRank[sk.confidence] > confRank[existing.confidence]
          ? sk.confidence
          : existing.confidence;
      // Combine or pick best evidence
      const bestEvidence =
        existing.evidence.length >= sk.evidence.length ? existing.evidence : sk.evidence;
      // Prefer non-OTHER category code
      const bestCategory =
        existing.category_code !== "OTHER" ? existing.category_code : sk.category_code;
...
export function detectHackathons(input: RawResumeInput): string[] {
  ...
  const hackathonRegex =
    /(hackathon|hackmit|hackath|codefest|devfest|ideathon|marathon|contest|1st place|finalist|winner)/i;
...
export function detectCertifications(input: RawResumeInput): string[] {
  ...
  const allFields = [
    ...
    ...(input.experience || []),
    ...(input.skills || []).map((s) =>
      typeof s === "string" ? s : `${s.skill_name} ${s.evidence}`,
    ),
  ];

  const certRegex =
    /(certified|certification|certificate|aws cert|azure cert|gcp cert|coursera|udemy|nptel|cka|ckad|cks|pmp|ccna)/i;
    ...
    if (certRegex.test(cleaned)) {
      if (cleaned.length > 80) {
        const match = cleaned.match(
          /([^,.!?:;]+\b(?:certified|certification|certificate|aws|azure|gcp|coursera|udemy|cka|pmp)\b[^,.!?:;]+)/i,
        );
...
export function buildCandidateProfile(input: RawResumeInput): Profile {
  ...
  if (input.projects) {
    for (const proj of input.projects) {
      ...
      if (
        (lower.includes("raft") || lower.includes("distributed")) &&
        !cleanSkills.some((s) => s.category_code === "SYSD")
      ) {
        cleanSkills.push({
      ...
      if (
        (lower.includes("sklearn") || lower.includes("churn")) &&
        !cleanSkills.some((s) => s.category_code === "AI")
      ) {
        cleanSkills.push({
  ...
  let internships =
    input.internships && input.internships.length > 0
      ? input.internships.map(cleanText).filter(Boolean)
      : detectInternships(input);
  ...
  if (!cgpa && education) {
    const match = education.match(
      /(\b\d+(?:\.\d+)?\s*\/\s*\d+(?:\.\d+)?\s*(?:cgpa|gpa)?|\b(?:cgpa|gpa|marks|percentage|score|grade)[:\s]*\d+(?:\.\d+)?(?:%|\/\d+)?|\b\d+(?:\.\d+)?\s*%\s*(?:marks)?)/i,
    );
    if (match) {
